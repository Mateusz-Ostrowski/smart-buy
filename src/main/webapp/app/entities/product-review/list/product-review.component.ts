...
export class ProductReviewComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.productReviewService
      ...
      .subscribe({
        next: (res: HttpResponse<IProductReview[]>) => {
  ...
  trackId(index: number, item: IProductReview): number {
    return item.id!;
  }

  delete(productReview: IProductReview): void {
    const modalRef = this.modalService.open(ProductReviewDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IProductReview[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
